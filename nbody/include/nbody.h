...
typedef struct
{
    ...
    int verbose;
    int abortNStep;     /* If > 0 and running with --use-cuda, voluntarily
                         * finish (exit 0, no result) any WU whose nStep is
                         * >= this value. 0 = disabled (default). Set via
                         * --abort-nsteps in app_info.xml <cmdline>. */
} NBodyFlags;
